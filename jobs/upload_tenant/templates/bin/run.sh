...
curl -X PUT \
  ...
  -H 'Content-Type: application/json' -d'{"index_permissions":[{"index_patterns":["logs-app-*"],"allowed_actions":["indices:monitor/settings/get"]}],"tenant_permissions":[{"tenant_patterns": ['"${org_quoted}"'],"allowed_actions": ["kibana_all_write"]}]}'
