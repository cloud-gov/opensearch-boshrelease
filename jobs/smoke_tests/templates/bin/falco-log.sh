...
INDEX="<%= index %>"
S3_BUCKET="<%= p('smoke_tests.s3_falco.bucket') %>"
...
# Upload log to S3
echo "Uploading Falco log to S3..."
if command -v aws &> /dev/null; then
    if [ -f "$S3_LOG_FILE" ]; then
        ...
        if [ $? -eq 0 ]; then
            echo "Successfully uploaded Falco log to s3://${S3_BUCKET}/${S3_KEY}"
            rm -f "$S3_LOG_FILE"
        else
    ...
    if [[ $result == *"$SMOKE_ID"* ]]; then
        ...
        space_opensearch=$(echo "$result" | jq -r '.hits.hits[0]._source["@cf"]["space_id"]')
         ...
         if [[ "$org_opensearch" == "$org_value" && "$space_opensearch" == "$space_value" ]]; then
            echo "SUCCESS: Falco log contains 'org id' and 'space id' fields."
            
            # Parse and validate Falco fields
            group_value=$(echo "$result" | jq -r '.hits.hits[0]._source["falco_logs"]["log_group"]')
            stream_value=$(echo "$result" | jq -r '.hits.hits[0]._source["falco_logs"]["log_stream"]')
            
            if [[ "$group_value" == "/aws/rds/instance/${rds_prefix}-tester/postgresql" && "$stream_value" == "${rds_prefix}-tester.0" ]]; then
                echo "SUCCESS: Falco log contains 'log group' and 'log stream' fields."
                exit 0
            else
                echo "ERROR: Falco log does not contain both 'log group' and 'log stream' fields."
                exit 1
            fi
        else
            echo "ERROR: Falco log does not contain both 'org id' and 'space id' fields."
            exit 1
        fi
    else
        sleep $SLEEP
...
# Timeout handling
echo -e "\nERROR: Timed out waiting for Falco log with $SMOKE_ID"
