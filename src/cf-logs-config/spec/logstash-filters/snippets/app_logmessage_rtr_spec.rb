...
describe "app-logmessage-rtr.conf" do
  ...
  describe "#fields when message is" do
    ...
    context "RTR format" do
      when_parsing_log(
          ...
          "@level" => "SOME LEVEL",
          # rtr format
          "@message" => "parser.127.0.0.1.xip.io - [15/07/2016:09:26:25 +0000] \"GET /some/http HTTP/1.1\" 200 0 1413 \"-\" \"Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36\" 127.0.0.1:35826 x_forwarded_for:\"127.0.0.1, 127.0.0.1\" x_forwarded_proto:\"http\" vcap_request_id:\"831e54f1-f09f-4971-6856-9fdd502d4ae3\" response_time:0.005328859 app_id:7ae227a6-6ad1-46d4-bfb9-6e60d7796bb5\n"
      ) do
        ...
        it "sets [rtr] fields" do
          expect(parsed_results.get("rtr")["hostname"]).to eq "parser.127.0.0.1.xip.io"
          ...
          expect(parsed_results.get("rtr")["x_forwarded_for"]).to eq ["127.0.0.1", "127.0.0.1"]
          ...
          # calculated values
          expect(parsed_results.get("rtr")["remote_addr"]).to eq "127.0.0.1"
        ...
        it "sets geoip for [rtr][remote_addr]" do
          ...
          expect(parsed_results.get("geoip")["ip"]).to eq "127.0.0.1"
        end
    ...
    context "RTR format (cf-release v250+)" do
      ...
      context "" do
        when_parsing_log(
            ...
            "@message" => "parser.127.0.0.1.xip.io - [15/07/2016:09:26:25 +0000] \"GET /some/http HTTP/1.1\" 200 0 1413 \"-\" \"Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36\" \"127.0.0.1:35826\" \"192.861.111.12:33456\" x_forwarded_for:\"127.0.0.1, 127.0.0.1\" x_forwarded_proto:\"http\" vcap_request_id:\"831e54f1-f09f-4971-6856-9fdd502d4ae3\" response_time:0.005328859 app_id:7ae227a6-6ad1-46d4-bfb9-6e60d7796bb5\n"
        ) do
    ...
    context "RTR format (cf-release v252+)" do
      ...
      context "" do
        when_parsing_log(
            ...
            "@level" => "SOME LEVEL",
            # rtr format - quoted requestRemoteAddr and destIPandPort
            "@message" => "parser.127.0.0.1.xip.io - [2017-03-16T13:28:25.166+0000] \"GET / HTTP/1.1\" 200 0 1677 \"-\" \"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/36.0.1985.67 Safari/537.36\" \"127.0.0.1:60079\" \"127.0.0.1:61010\" x_forwarded_for:\"127.0.0.1, 127.0.0.1\" x_forwarded_proto:\"https\" vcap_request_id:\"f322dd76-aacf-422e-49fb-c73bc46ce45b\" response_time:0.001602684 app_id:\"27c02dec-80ce-4af6-94c5-2b51848edae9\" app_index:\"1\"\n"
        ) do
          ...
          it "sets [rtr] fields" do
            ...
            expect(parsed_results.get("rtr")["src"]["host"]).to eq "127.0.0.1"
            expect(parsed_results.get("rtr")["src"]["port"]).to eq 60079
            expect(parsed_results.get("rtr")["dst"]["host"]).to eq "127.0.0.1"
      ...
      context "empty requestRemoteAddr and destIPandPort" do
        when_parsing_log(
            ...
            "@level" => "SOME LEVEL",
            # rtr format - quoted requestRemoteAddr and destIPandPort
            "@message" => "parser.127.0.0.1.xip.io - [15/07/2016:09:26:25 +0000] \"GET /some/http HTTP/1.1\" 200 0 1413 \"-\" \"Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36\" \"-\" \"-\" x_forwarded_for:\"127.0.0.1, 127.0.0.1\" x_forwarded_proto:\"http\" vcap_request_id:\"831e54f1-f09f-4971-6856-9fdd502d4ae3\" response_time:0.005328859 app_id:7ae227a6-6ad1-46d4-bfb9-6e60d7796bb5\n"
        ) do
    ...
    context "RTR format (cf-deployment v12.17.0+)" do
      ...
      context "" do
        when_parsing_log(
            ...
            "@level" => "SOME LEVEL",
            # rtr format - quoted requestRemoteAddr and destIPandPort
            "@message" => "parser.127.0.0.1.xip.io - [2017-03-16T13:28:25.166+0000] \"GET / HTTP/1.1\" 200 0 1677 \"-\" \"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/36.0.1985.67 Safari/537.36\" \"127.0.0.1:60079\" \"127.0.0.1:61010\" x_forwarded_for:\"127.0.0.1, 127.0.0.1\" x_forwarded_proto:\"https\" vcap_request_id:\"f322dd76-aacf-422e-49fb-c73bc46ce45b\" response_time:0.001602684 gorouter_time:0.000163 app_time:0.024988 app_id:\"27c02dec-80ce-4af6-94c5-2b51848edae9\" app_index:\"1\"\n"
        ) do
  ...
  describe "when HTTP status" do
    ...
    context "<400 (INFO @level)" do
      when_parsing_log(
          ...
          "@source" => { "type" => "RTR" },
          "@level" => "SOME LEVEL",
          "@message" => "parser.127.0.0.1.xip.io - [15/07/2016:09:26:25 +0000] \"GET /http HTTP/1.1\" " +
              "200" + # HTTP status <400
  ...
  describe "when [rtr][x_forwarded_for]" do
    ...
    context "contains quotes & whitespaces" do
      when_parsing_log(
          ...
          "@source" => { "type" => "RTR" },
          "@level" => "SOME LEVEL",
          "@message" => "parser.127.0.0.1.xip.io - [15/07/2016:09:26:25 +0000] \"GET /http HTTP/1.1\" 200 0 1413 \"-\" \"Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36\" 127.0.0.1:35826 " +
              "x_forwarded_for:\"\"  127.0.0.1 \",     127.0.0.1 \"" +  # contains quotes & whitespaces
  ...
  describe "when [rtr][remote_addr]" do
    ...
    context "has ip format" do
      when_parsing_log(
          ...
          "@level" => "SOME LEVEL",
          "@message" => "parser.127.0.0.1.xip.io - [15/07/2016:09:26:25 +0000] \"GET /http HTTP/1.1\" 200 0 1413 \"-\" \"Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36\" 127.0.0.1:35826 " +
              "x_forwarded_for:\"127.0.0.1\"" + # ip format
              " x_forwarded_proto:\"http\" vcap_request_id:\"831e54f1-f09f-4971-6856-9fdd502d4ae3\" response_time:0.005328859 app_id:7ae227a6-6ad1-46d4-bfb9-6e60d7796bb5\n"
      ) do

        it { expect(parsed_results.get("rtr")["remote_addr"]).to eq "127.0.0.1" }
    ...
    context "has bad format" do
      when_parsing_log(
          ...
          "@level" => "SOME LEVEL",
          "@message" => "parser.127.0.0.1.xip.io - [15/07/2016:09:26:25 +0000] \"GET /http HTTP/1.1\" 200 0 1413 \"-\" \"Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36\" 127.0.0.1:35826 " +
              "x_forwarded_for:\"bad_format, 127.0.0.1\"" + # bad format
              " x_forwarded_proto:\"http\" vcap_request_id:\"831e54f1-f09f-4971-6856-9fdd502d4ae3\" response_time:0.005328859 app_id:7ae227a6-6ad1-46d4-bfb9-6e60d7796bb5\n"
      ) do
  ...
  describe "when [rtr][response_time_sec] has only a few digits" do
    when_parsing_log(
        ...
        "@level" => "SOME LEVEL",
        # rtr format
        "@message" => "parser.127.0.0.1.xip.io - [15/07/2016:09:26:25 +0000] \"GET /some/http HTTP/1.1\" 200 0 1413 \"-\" \"Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36\" 127.0.0.1:35826 x_forwarded_for:\"127.0.0.1, 127.0.0.1\" x_forwarded_proto:\"http\" vcap_request_id:\"831e54f1-f09f-4971-6856-9fdd502d4ae3\" response_time:0.5 app_id:7ae227a6-6ad1-46d4-bfb9-6e60d7796bb5\n"
    ) do
  ...
  describe "when [rtr][response_time_sec] is an integer" do
    when_parsing_log(
        ...
        "@level" => "SOME LEVEL",
        # rtr format
        "@message" => "parser.127.0.0.1.xip.io - [15/07/2016:09:26:25 +0000] \"GET /some/http HTTP/1.1\" 200 0 1413 \"-\" \"Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36\" 127.0.0.1:35826 x_forwarded_for:\"127.0.0.1, 127.0.0.1\" x_forwarded_proto:\"http\" vcap_request_id:\"831e54f1-f09f-4971-6856-9fdd502d4ae3\" response_time:5 app_id:7ae227a6-6ad1-46d4-bfb9-6e60d7796bb5\n"
    ) do
