  ...
  describe "#fields when message is" do
    context "Http format" do
      when_parsing_log(
          ...
          # http format
          "@message" => "127.0.0.18:60677 [06/Jul/2016:13:59:57.770] https-in~ http-routers/node0 59841/0/0/157/60000 200 144206 reqC respC ---- 3/4/1/2/0 5/6 {reqHeaders} {respHeaders} \"GET /v2/apps?inline-relations-depth=2 HTTP/1.1\""
      ) do
    ...
    context "Http format (<BADREQ>)" do
      when_parsing_log(
          "@source" => {"component" => "haproxy"},
          # http format
          "@message" => "127.0.0.18:60677 [06/Jul/2016:13:59:57.770] https-in~ http-routers/node0 59841/0/0/157/60000 " +
    ...
    context "Error log format" do
      when_parsing_log(
          "@source" => {"component" => "haproxy"},
          # error log
          "@message" => "127.0.0.1:36743 [06/Jul/2016:07:16:34.605] https-in/1: SSL handshake failure"
      ) do
        ...
        it "sets [haproxy] fields from grok" do
          expect(parsed_results.get("haproxy")["client_ip"]).to eq "127.0.0.1"
