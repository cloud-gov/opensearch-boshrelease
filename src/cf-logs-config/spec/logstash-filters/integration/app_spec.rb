    ...
    describe "LogMessage (RTR)" do
      ...
      sample_event["@message"] = construct_event("LogMessage", true,
                                                 {"source_type" => "RTR", # RTR
                                                  ...
                                                  "level" => "debug",
                                                  # RTR message
                                                  "msg" => 'parser.127.0.0.1.xip.io - [2017-03-16T13:28:25.166+0000] \"GET / HTTP/1.1\" ' +
                                                           '200 0 1677 \"-\" \"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/36.0.1985.67 Safari/537.36\" ' +
                                                           '\"127.0.0.1:60079\" \"127.0.0.1:61010\" x_forwarded_for:\"127.0.0.1, 127.0.0.1\" x_forwarded_proto:\"https\" ' +
    ...
    describe "HttpStartStop" do
      ...
      when_parsing_log(sample_event) do
        ...
        verify_app_general_fields("app", "HttpStartStop", "HTTP",
                                  "200 GET http://127.0.0.1/internal/v3/bulk/task_states (6 ms)", "INFO")
        ...
        it "sets [httpstartstop] fields" do
          ...
          expect(parsed_results.get("httpstartstop")["remote_addr"]).to eq "127.0.0.1:42801"
          ...
          expect(parsed_results.get("httpstartstop")["uri"]).to eq "http://127.0.0.1/internal/v3/bulk/task_states"
