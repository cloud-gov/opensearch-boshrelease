    ...
    context "UAA Audit" do
      context "(general event)" do
        when_parsing_log(
          "@source" => {"component" => "vcap.uaa"},
          # general UAA event
          "@message" => "[2016-07-05 04:02:18.245] uaa - 15178 [http-bio-8080-exec-14] ....  INFO --- Audit: ClientAuthenticationSuccess ('Client authentication success'): principal=cf, origin=[remoteAddress=127.0.0.18, clientId=cf], identityZoneId=[uaa]"
        ) do
      ...
      context "(PrincipalAuthFailure event)" do
        when_parsing_log(
            ...
            "@message" => "[2016-07-06 09:18:43.397] uaa - 15178 [http-bio-8080-exec-6] ....  INFO --- Audit: " +
                "PrincipalAuthenticationFailure ('null'): principal=admin, origin=[127.0.0.1], identityZoneId=[uaa]"
        ) do
          ...
          it "sets [uaa][audit] fields" do
            ...
            expect(parsed_results.get("uaa")["audit"]["origin"]).to eq ["127.0.0.1"]
            expect(parsed_results.get("uaa")["audit"]["identity_zone_id"]).to eq "uaa"
            expect(parsed_results.get("uaa")["audit"]["remote_address"]).to eq "127.0.0.1"
          end
