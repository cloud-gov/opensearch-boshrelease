  ...
  describe "#fields when message is" do
    ...
    context "JSON format" do
      when_parsing_log(
          ...
          # JSON format
          "@message" => "{\"timestamp\":1467852972.554088,\"source\":\"NatsStreamForwarder\",\"log_level\":\"info\",\"message\":\"router.register\",\"data\":{\"nats_message\": \"{\\\"uris\\\":[\\\"redis-broker.127.0.0.1.xip.io\\\"],\\\"host\\\":\\\"127.0.0.1\\\",\\\"port\\\":80}\",\"reply_inbox\":\"_INBOX.7e93f2a1d5115844163cc930b5\"}}"
      ) do
        ...
        it "sets JSON fields" do
          ...
          expect(parsed_results.get("parsed_json_field")["data"]["nats_message"]).to eq "{\"uris\":[\"redis-broker.127.0.0.1.xip.io\"],\"host\":\"127.0.0.1\",\"port\":80}"
