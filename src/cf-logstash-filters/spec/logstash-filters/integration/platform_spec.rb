...
describe "Platform IT" do
  ...
  describe "when CF (metron agent) and format is" do
    ...
    context "vcap (json)" do
      ...
      message_payload = MessagePayloadBuilder.new
                            ...
                            .message_text('{"timestamp":1467852972.554088,"source":"NatsStreamForwarder", ' +
                                              ...
                                              '"data":{"nats_message": "{\"uris\":[\"redis-broker.127.0.0.1.xip.io\"],\"host\":\"127.0.0.1\",\"port\":80}",' +
      ...
      when_parsing_log(sample_event) do
        ...
        it "sets fields from JSON" do
          ...
          expect(parsed_results.get("consul_agent")["data"]["nats_message"]).to eq "{\"uris\":[\"redis-broker.127.0.0.1.xip.io\"],\"host\":\"127.0.0.1\",\"port\":80}"
    ...
    context "haproxy" do
      message_payload = MessagePayloadBuilder.new
                            .job("ha_proxy_z1")
                            .message_text('127.0.0.18:60677 [06/Jul/2016:13:59:57.770] https-in~ http-routers/node0 59841/0/0/157/60000 200 144206 reqC respC ---- 3/4/1/2/0 5/6 {reqHeaders} {respHeaders} "GET /v2/apps?inline-relations-depth=2 HTTP/1.1"')
                            .build()
      sample_event = platform_event_dummy.clone
      sample_event["@message"] = construct_cf_message__metronagent_format(message_payload)
      ...
      when_parsing_log(sample_event) do
        ...
        it "sets [haproxy] fields from grok" do
          expect(parsed_results.get("haproxy")["client_ip"]).to eq "127.0.0.18"
    ...
    context "uaa (Audit)" do
      message_payload = MessagePayloadBuilder.new
                            .job("uaa_z0")
                            .message_text('[2016-07-05 04:02:18.245] uaa - 15178 [http-bio-8080-exec-14] ....  INFO --- Audit: ClientAuthenticationSuccess (\'Client authentication success\'): principal=cf, origin=[remoteAddress=127.0.0.18, clientId=cf], identityZoneId=[uaa]')
                            .build()
      sample_event = platform_event_dummy.clone
      sample_event["@message"] = construct_cf_message__metronagent_format(message_payload)
      sample_event["syslog_program"] = "vcap.uaa" # uaa
      ...
      when_parsing_log(sample_event) do
        ...
        it "sets [uaa][audit] fields" do
          ...
          expect(parsed_results.get("uaa")["audit"]["origin"]).to eq ["remoteAddress=127.0.0.18", "clientId=cf"]
          expect(parsed_results.get("uaa")["audit"]["identity_zone_id"]).to eq "uaa"
          expect(parsed_results.get("uaa")["audit"]["remote_address"]).to eq "127.0.0.18"
        end

        it "sets geoip for remoteAddress" do
          expect(parsed_results.get("geoip")).not_to be_nil
          expect(parsed_results.get("geoip")["ip"]).to eq "127.0.0.18"
        end
